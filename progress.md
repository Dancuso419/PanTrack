...
| 2026-07-18 | Mobile/device optimization pass | Verified every page at 390px + 768px + 1440px via headless screenshots (no horizontal overflow anywhere). Fixed iOS auto-zoom on input focus (form controls forced to 16px on mobile via `!important` to beat text-sm — verified computed 16px). Added `viewport-fit=cover` + safe-area-inset padding (header top / main bottom) for notched phones; theme-color + apple mobile-web-app meta; removed tap-highlight flash; overscroll-contain on main scroll; bigger delete-icon touch targets. |
